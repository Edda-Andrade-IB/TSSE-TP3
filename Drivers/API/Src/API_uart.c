...
/**
 * Possible states of the uart buffer.
 */
typedef enum {
  ACCUMULATING,
  DONE,
...
/**
 * Append a char to the string. The function `strcat` forces you tu pass a char
 * * pointer as argument, this one only receives a char.
 */
...
bool_t uartInit() {
  ...
  if (HAL_UART_Init(&UartHandle) == HAL_OK) {
    ...
    sprintf(message, conf_message, BAUD_RATE, WORD_LENGTH, STOP_BITS, parity,
            flow_constrol);
...
void USART3_IRQHandler(void) { HAL_UART_IRQHandler(&UartHandle); }
